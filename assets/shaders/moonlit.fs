...
extern PRECISION vec2 hotsh;
extern PRECISION vec2 moonlit;
...
vec4 increase_contrast(vec4 tex, float contrast_level)
{
    ...
    return vec4(adjusted_rgb, tex.a);
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    tex.rgb = mix(tex.rgb, gray.rgb, 0.9);
    ...
    if (hotsh.g > 0.0 || hotsh.g < 0.0 || moonlit.g > 0.0 || moonlit.g < 0.0) {
        cyan *= 1.3;
        magenta *= 1.1;
        yellow *= 0.7;
        black *= 1.4;
    ...
    tex.r = (1 - cyan) * (1 - black);
    tex.g = (1 - magenta) * (1 - black);
    tex.b = (1 - yellow) * (1 - black);

    float brightness_reduction = (0.8 * (uv.y-0.5));
    float bottom_rim = (0.1 * (uv.y-0.5))/5;
    float x_brightness_reduction = (0.6 * (uv.x-0.5));
    tex.rgb *= (1.0 - brightness_reduction);
    tex.rgb *= (1.0 + bottom_rim);
    tex.rgb *= (1.0 - x_brightness_reduction);
    tex.b *= (1.0 + (0.3 * brightness_reduction));
    tex.b *= (1.0 + (0.2 * x_brightness_reduction));

    tex = increase_contrast(tex, 1.5);
